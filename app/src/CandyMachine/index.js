...
const CandyMachine = ({ walletAddress }) => {
	...
	const getCandyMachineState = async () => {
		...
		// both idl and program are like a database connection
		// idl has information that our web app needs to interact with the candy machine
		const idl = await Program.fetchIdl(candyMachineProgram, provider);
		// the program allows us to interact directly with the candy machine
		const program = new Program(idl, candyMachineProgram, provider);
		// specify which candy machine we want to retrieve. use solana devnet to retrieve data
		...
		// if whitelistMintSettings === true AND presale === true check if goLiveDate === false OR goLiveDate is greater then current date and time
		const presale =
	...
	const mintToken = async () => {
		...
		// check if there is a whitelist setup
		...
		// check if mint is token-gated. This means an NFT has private data and only the holder of that NFT will have access to that private data.
		if (candyMachine.state.tokenMint) {
			...
			instructions.push(
				Token.createApproveInstruction(TOKEN_PROGRAM_ID, userPayingAccountAddress, transferAuthority.publicKey, walletAddress.publicKey, [], candyMachine.state.price.toNumber())
			);
	...
	return (
		candyMachine && (
			<div className='machine-container'>
				...
				<p>
					Items Minted: {candyMachine.state.itemsRedeemed} / {candyMachine.state.itemsAvailable}
				</p>
